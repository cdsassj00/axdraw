...
npm run test:e2e # 브라우저 자동화 테스트 87개
```
...
**펜은 기본적으로 손그림 그대로 남습니다.** 자동 인식을 원하면 오른쪽 위 지팡이 버튼으로 켜거나(켜면 손을 떼는 순간 분류), 그려둔 스트로크를 **우클릭 → "Convert to shape"** 로 원할 때만 변환할 수 있습니다.
...
**그림 찾기** — 좌측 하단 돋보기 옆의 조준 버튼. 캔버스가 커지면 "전체 맞춤"은 그림 사이의 빈 공간까지 맞추게 되어 화면이 점들로 가득 찹니다. 이 버튼은 떨어져 있는 그림 덩어리를 **요소가 많은 순서로** 나열하고, 고르면 그리로 이동하며 선택까지 해 줍니다. 요소 1000개가 넘어도 즉시 계산됩니다(격자 버킷 방식). 덩어리가 하나뿐이면 목록 없이 바로 전체 맞춤으로 동작합니다.

**다른 캔버스로 이동** — 요소를 선택하고 우클릭 > "다른 캔버스로 이동…". 한 캔버스에 뒤섞인 작업을 덩어리별로 갈라낼 때 씁니다. 좌표를 그대로 유지하므로 옮긴 쪽에서도 배치가 같습니다. 옮긴 직후 실행 취소하면 원본이 돌아오고 **옮긴 캔버스의 사본도 함께 사라집니다** — 양쪽에 남아 조용히 복제되는 일이 없습니다. 다만 이후 다른 편집을 하면 그 되돌리기는 더 이상 반대편 캔버스를 건드리지 않습니다.

**협업 방은 저장됩니다** — 방의 장면이 몇 초마다 암호화되어 클라우드플레어에 기록되고, 다시 들어오면 복원됩니다. 그래서 **협업 링크 하나면 충분합니다.** 예전에는 방이 중계만 해서 마지막 사람이 나가면 그림이 사라졌고, 남기려면 공유 링크를 따로 만들어야 했는데 그건 만든 순간에 얼어붙어서 이후 작업을 담지 못했고 다시 만들면 또 다른 링크가 생겼습니다. 저장되는 것은 여전히 **암호문**이며 키는 링크의 `#` 뒤에만 있습니다 — 서버는 내용을 읽을 수 없습니다.

저장 위치는 **R2**입니다(`ROOM_SCENES` 바인딩). 바인딩이 없으면 KV로 폴백하므로 바인딩을 지워도 동작은 합니다 — 다만 KV는 무료 플랜에서 하루 1,000회 쓰기 제한이 있고, 무엇보다 **최종 일관성**이라 방금 쓴 내용이 읽히기까지 실측 약 40초가 걸립니다. 그 사이에 다른 기기에서 들어오면 빈 방으로 보이고, 거기서 그리면 저장된 내용을 덮어쓸 수 있습니다. 그래서 KV로 동작할 때는 조회가 비면 약 10초간 재시도하고 **그 전까지 저장을 막습니다**(덮어쓰기 방지). R2는 쓴 즉시 읽히므로 이 지연도 가드도 없습니다.
...
**협업 방은 자기 캔버스를 가집니다** — 협업 링크로 들어가면 방 전용 캔버스가 새로 열립니다. 예전에는 열려 있던 캔버스를 그대로 들고 들어가서 첫 브로드캐스트에 그 내용이 방으로 밀려 들어갔고, 수업에서 참가자 각자의 개인 메모가 공유 화면에 겹쳐 쌓였습니다. 이제 내 작업은 내 캔버스에 그대로 남고, 방은 참가자들이 보내주는 내용에서 시작합니다. 같은 방에 다시 들어가면 그 방의 캔버스로 돌아갑니다.

**최근 협업 방** — 메뉴 > "최근 협업 방". 협업 방은 서버에 저장되지 않는 **중계**라서, 링크가 주소창 기록에서 사라지면 다시 들어갈 방법이 없습니다(복호화 키가 링크의 `#` 뒤에만 있기 때문). 그래서 방문한 방을 최근 12개까지 브라우저에 기록해 두고, 목록에서 바로 재접속하거나 링크를 복사할 수 있게 했습니다. 이 목록은 해당 브라우저에만 있으며 서버로 나가지 않습니다 — 백업이 아니라 편의 기능입니다. 내용까지 보관하려면 **공유 링크**를 쓰세요.
...
**스타일** — 획/배경 색상(빠른 팔레트 + 130색 피커 + 커스텀 HEX), 채움(빗금·교차빗금·단색·지그재그), 획 굵기 3단계, 실선/파선/점선, 손그림 거칠기 3단계(architect/artist/cartoonist), 모서리(각짐/둥글게), 투명도, 화살촉 7종, 글꼴 6종(손글씨 Caveat·Gaegu / 시스템 / 코드 / Pretendard / Noto Sans KR / Noto Serif KR, 웹폰트는 CDN 로드)·굵기 1~20px 슬라이더·크기 4단계·정렬.
...
테스트는 `dist/`를 대상으로 돌기 때문에 **`npm run build`가 통과했는지 반드시 확인**하고 실행하세요. 빌드가 tsc 오류로 멈추면 `dist/`에 이전 번들이 그대로 남아, 고치지도 않은 코드로 테스트가 통과합니다. 또 Playwright가 번들한 크로미움과 설치된 버전이 어긋나면 `PLAYWRIGHT_CHROMIUM`으로 실행 파일을 직접 지정할 수 있습니다:

```bash
PLAYWRIGHT_CHROMIUM=/opt/pw-browsers/chromium-<빌드>/chrome-linux/chrome npm run test:e2e
```
