...
저장 위치는 R2 버킷이 연결되어 있으면 R2, 아니면 KV입니다. **R2를 권장합니다** — KV는 무료 플랜에서 하루 1,000회 쓰기 제한이 있고, 무엇보다 **최종 일관성**이라 방금 쓴 내용이 읽히기까지 실측 약 40초가 걸립니다. 그 사이에 다른 기기에서 들어오면 빈 방으로 보이고, 거기서 그리면 저장된 내용을 덮어쓸 수 있습니다. 그래서 KV일 때는 조회가 비면 약 10초간 재시도하고 **그 전까지 저장을 막습니다** (덮어쓰기 방지). R2는 쓴 즉시 읽히므로 이 지연이 없습니다:
